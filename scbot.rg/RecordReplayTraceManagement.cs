    ...
    public class RecordReplayTraceManagement : RegexCommandMessageProcessor
    {
        ...
        private static MessageResult DeleteTracesFor(Message message, Match args)
        {
            ...
            try
            {
                ...
                return new MessageResult(new[] { Response.ToMessage(message, "Deleted " + path) });
            }
            catch (Exception e)
            {
                return new MessageResult(new[] { Response.ToMessage(message, "Failed to delete " + path + ": " + e.Message) });
            }
        }
